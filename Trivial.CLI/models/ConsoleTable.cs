...
public class ConsoleTable
{
    ...
    private string _GenerateHeader()
    {
        var t_HeaderStr = "";
        Headers.ForEach((S, I) => {
            t_HeaderStr += S.Length < m_ColumnWidths[I] ? S + _CreatePadString(m_ColumnWidths[I] - S.Length) : S.Substring(0, m_ColumnWidths[I]);
        });
        return t_HeaderStr;
    }
    ...
    private string _CreatePadString(int Count) => 
        string.Join("", Enumerable.Range(0, Count).Select(_ => " "));
}
