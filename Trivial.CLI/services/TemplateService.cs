...
public class TemplateService(ITemplateRepository Repo, ISearchService SearchService, IRepoService RepoService) : ITemplateService
{
    ...
    public Result<Unit> InstallTemplateFromRepo(string Name, bool Force)
    {
        ...
        var t_Repo = RepoService.GetLocalIndexes().FirstOrNone(I => I.Templates.Any(T => T.Id == t_Template.Id));
        if(!t_Repo.HasValue) return new Exception($"Repo not found for template {Name}");

        if(!RepoService.IsTemplateInCacheById(t_Repo.Value.Id, Guid.Parse(t_Template.Id)))
        {
            var t_Result = RepoService.CacheTemplateFromRemote(Guid.Parse(t_Template.Id), t_Repo.Value.Id);
            if(t_Result.HasError) return t_Result;
        }

        var t_Path = RepoService.GetCachedTemplatePath(t_Repo.Value.Id, Guid.Parse(t_Template.Id));
